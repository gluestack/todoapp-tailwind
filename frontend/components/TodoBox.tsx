...
import { useUpdateTodoMutation, useDeleteTodoMutation } from '../services/__generated__';
...
const TodoBox = ({ todo }: { todo: ITodo }) => {
  ...
  const checkedHandler = async () => {
    try {
      //@ts-ignore
      const { data } = await updateTodoGQL({ variables: { id: todo.id, title: todo.title, is_completed: !todo.is_completed } });
  ...
  return (
    <div className={`relative ${styles.todo}`}>
      ...
      <span onClick={deleteHandler} className={`${styles.delete} absolute right-0 bottom-4 text-2xl mr-3 cursor-pointer`} ><MdDeleteForever /></span>
    </div>
  )
}
