...
const TodosFooter = () => {
  ...
  React.useEffect(() => {
    const completeTodos = todos.filter((todo: { is_completed: any; }) => todo.is_completed);
    ...
    <div className='h-10 flex items-center justify-between text-sm font-thin px-3 select-none'>
      ...
      <div className='flex gap-2'>
        {['All', 'Active', 'Completed'].map((value, i) => (
          <span key={i} className={`border ${filter === value.toLowerCase() && activeBorder} px-2 cursor-pointer`} onClick={() => activeHandler(value.toLowerCase())}>
            {value}
          </span>
        ))}
      </div>
      <span className={`cursor-pointer ${!markCompleted && 'invisible'}`} onClick={deleteTodos}>Clear completed</span>
    </div>
  )
}
