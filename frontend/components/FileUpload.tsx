...
const FileUpload = (props: any) => {
  ...
  const handleUpload = async (file: any) => {
    try {
      ...
      const { data } = await insertFileId({ variables: { id: props.id, file_id: response.id, file_path: response.path } })
      if (data?.update_todos_by_pk) {
        attachFile({ id: props.id, file_id: response.id, file_path: response.path })
      }

      toggleToast({ success: true, display: true, message: "successfully uploaded!" });
  ...
  return (
    <>
      <input type="file" ref={fileInput} onChange={handleChange} style={{ display: "none" }} accept="image/jpeg, image/png" />
